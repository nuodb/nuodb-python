...
class Cursor(object):
    ...
    def fetchone(self):
        ...
        if not rs.is_complete():
            self.session.fetch_result_set_next(rs)
        return rs.fetchone()
    ...
    def fetchall(self):
        ...
        all_rows = []  # type: List[result_set.Row]
        while True:
            # Drain the current in-memory batch in one shot instead of calling
            # fetchone() per row.  This eliminates 515k Python function calls
            # for a typical full-table scan.
            idx = rs.results_idx
            batch = rs.results
            n = len(batch)
            if idx < n:
                all_rows.extend(batch[idx:n])
                rs.results_idx = n
            if rs.complete:
                break
            self.session.fetch_result_set_next(rs)
        self.rownumber += len(all_rows)
        return all_rows
