...
def waitforsql(tmout):
    ...
    try:
        while True:
            try:
                conn = pynuodb.connect(database=DATABASE_NAME, host=host,
...
def setUpModule_pynuoadmin():
    ...
    for db in ap_conn.get_databases():
        if db.name == DATABASE_NAME:
            logging.critical("Reusing already-existing database %s" % (DATABASE_NAME))
            break

    # If we have a database and it's running, we're all set!
    if db and db.state != 'NOT_RUNNING':
        logging.critical("Reusing already-running database %s" % (db.state))
        return
    ...
    def create_archive():
        ...
        logging.critical("Creating archive %s" % (ar_path))
        ap_conn.create_archive(DATABASE_NAME, apid, ar_path)

    # If we have a database and it's not running, start it
    if db:
        logging.critical('Starting NOT_RUNNING database %s' % (db.state))
        ...
        logging.critical("Creating database %s" % (DATABASE_NAME))
        create_archive()
    ...
    db_started = True
    logging.critical("Wait for running: state %s" % (db.state))
    end = time.time() + 30
...
def waitforstate(state, tmout):
    logging.critical("Waiting %ds for state %s" % (tmout, state))
    end = time.time() + tmout
    while True:
        ...
        if db.get('state') == state:
            logging.critical("DB is %s" % (state))
            return
        if time.time() > end:
...
def setUpModule_nuocmd():
    ...
    if ret == 0:
        ...
        if db and db.get('state') != 'TOMBSTONE':
            logging.critical("Using existing database %s" % (DATABASE_NAME))
            has_db = True
            if db.get('state') == 'RUNNING':
                logging.critical("Database is already RUNNING")
                return
